...
def init_db() -> None:
    ...
    ensure_column(conn, "monitors", "behavior_metadata", "text")
...
def api_create_monitor():
    ...
    cur = conn.execute(
        ...
        insert into monitors(
            ...
            proxy_url, session_task_key, session_metadata, behavior_metadata, created_at
        )
        values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            ...
            proxy_url,
            session_task_key,
            json.dumps(session_metadata) if session_metadata is not None else None,
            json.dumps(behavior_metadata) if behavior_metadata is not None else None,
            utc_now(),
        ),
    )
    conn.commit()
